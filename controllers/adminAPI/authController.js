...
exports.signin = async (req, res) => {
    try {
        ...
        if (admin_info && admin_info.role === 'administrator') {
            ...
            const { _id, firstname, lastname, fullname, email, role } = admin_info;
            if (!isMatch) {
                res.status(400).json({ message: "Invalid email and password." });
            } else {
                res.status(200).json({ _id, token, data: { _id, firstname, lastname, fullname, role, email }, message: "Admin Signin Successfully" });
            }
        } else {
